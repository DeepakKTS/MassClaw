...
class TestAutoShadow:
    @pytest.mark.asyncio
    async def test_unknown_workflow_is_shadowed_and_record_persists(
        self, db_session, redis_client, originator_keypair
    ):
        ...
        known = await gossip._load_known_workflow_ids()
        report = GossipRoundReport(
            peer_url="test", peer_did="test", duration_ms=0.0,
            buckets_compared=0, buckets_divergent=0,
            hashes_missing_locally=0, records_fetched=0, records_persisted=0,
        )
        ...
        shadow = (
            await db_session.execute(select(Workflow).where(Workflow.workflow_id == workflow_id))
        ).scalar_one()
        ...
        record_count = (
            await db_session.execute(
                select(MemoryRecord).where(MemoryRecord.workflow_id == workflow_id)
            )
        ).scalars().all()
    ...
    @pytest.mark.asyncio
    async def test_known_workflow_no_shadow_created(
        self, db_session, redis_client, originator_keypair
    ):
    ...
    @pytest.mark.asyncio
    async def test_auto_shadow_is_idempotent(
        self, db_session, redis_client, originator_keypair
    ):
