...
class Project(Base, AuditMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, server_default=text("'active'"), index=True,
    )
...
class BacklogTask(Base, AuditMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, server_default=text("'new'"), index=True,
    )
...
class Sprint(Base, AuditMixin):
    ...
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, server_default=text("'planning'"), index=True,
    )
...
class SprintTask(Base, AuditMixin):
    ...
    sprint_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("sprints.sprint_id", ondelete="CASCADE"),
        nullable=False, index=True,
    )
    backlog_task_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("backlog_tasks.backlog_task_id", ondelete="CASCADE"),
        nullable=False, index=True,
    )
    workflow_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("workflows.workflow_id", ondelete="SET NULL"),
        nullable=True,
    )
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, server_default=text("'todo'"), index=True,
    )
