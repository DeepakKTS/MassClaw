.PHONY: help dev backend frontend celery-worker celery-beat \
	...
	test test-db-setup test-unit test-integration test-e2e test-cov test-frontend \
...
# means CI has nothing new to tell you. One check is stricter than CI on
# purpose and is labelled [extra]: mypy being visible rather than swallowed.
#
...
# pass while CI failed, and vice versa. The frontend typecheck is now a real
# CI step, so it is no longer [extra]. Keep the two in sync when either
# changes.
#
...
ci-local: ## Run every CI check locally
	...
	@echo "=== Frontend: typecheck ===" && cd frontend && npm run typecheck
