...
export function RecentWorkflows() {
  ...
  return (
    <div className="bg-massclaw-surface border border-massclaw-border rounded-lg p-5">
      <h2 className="text-sm font-semibold mb-4">Recent Workflows</h2>
      {isLoading ? (
        <p className="text-massclaw-text-muted text-sm">Loading...</p>
      ) : !data?.items?.length ? (
        <p className="text-massclaw-text-muted text-sm">No workflows yet</p>
      ) : (
        <div className="space-y-3">
          {data.items.slice(0, 5).map((wf) => (
            <Link key={wf.workflow_id} href={`/workflows/${wf.workflow_id}`} className="block hover:bg-massclaw-border/20 rounded p-2 -mx-2 transition">
